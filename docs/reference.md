...
Worker skill の実装形態のひとつ。Worker は通常 1 つの Sub Agent をワークフロー内で単発起動するが、Agent Team を使うと複数の teammate を長期に並行生存させ、互いにメッセージをやりとりしながら作業を進められる。
...
- **メンバー間の往復メッセージが必要**: レビュー → 修正 → 再レビューのような反復サイクル
- **複数 agent の並行稼働にメリットがある**: 独立視点の同時確保、並列生成、並列探索など

編成パターンは用途次第で、レビューサイクル（作成者 + レビュアー）、並列生成と統合（同役割 × N + 統合者）、並列探索（同役割 × N で別々の仮説を追う）などに流用できる。単発の Sub Agent 呼び出しで足りるならチームは作らない。

### team skill の固定骨格

team を起動する skill は Worker skill の一種で、`user-invocable: false` を指定してオーケストレーターから呼ばれる前提で作る。以下の骨格はどの編成でも共通する。
...
ファイル名は書かず、セマンティック名（例: `実装計画のファイルパス`）で列挙する。
...
### 共通ルール
- 各メンバーは定義された名前（{member名を列挙}）と完全一致する name でスポーンすること
...
subagent_type: `{skillName}-{member-name}` でスポーンすること。役割と手順は `agents/{skillName}-{member-name}.md` を参照。
```
...
- **確認応答方式**: 受信側は先に ack を返してから作業を開始する。ack が返らない場合は最大 5 回まで再送する
- **ワークフローファイル経由**: 成果物は `~/claude-code-data/workflows/{タスクID}/{名前}.md` に保存し、メッセージではファイルパスだけを渡す
- **ファイル名の所有権**: ファイル名は書き出す側の member だけが知る。他 member やリーダーはセマンティック名（「計画ファイルのパス」）で受け取り、通知された文字列をそのまま使う

これらは「共通パターン」節の「Agent間の情報受け渡し（ワークフローファイル経由）」と同じ原則を team 内に適用したもの。

### アンチパターン

- **リーダーが member の作業を奪う**: リーダーは進行管理のみ。実装・レビュー・実行は member の責務に委ねる
- **member が他 member の出力ファイル名をハードコード**: 必ず通知で受け取ったパスを使う
- **ack なしの一方通行メッセージ**: 確認応答方式を守らないと「届いていない」事態を検知できない
- **チーム削除忘れ**: 全員完了後にリーダーが必ずチームを削除する
- **レビューサイクルを組む場合、打ち切り上限なし**: 最大 N 回で打ち切り、超えたらユーザーに判断を仰ぐ
...
レビュー → 修正 → 再レビューのようなサイクルは、メンバー間のメッセージ往復で実現する。初回呼び出しで得た識別子を使って前回のコンテキストを保持したまま再依頼することで、コンテキストを毎回作り直さずに継続できる。
