...
- `HookAction` を `command` / `http` / `mcp_tool` / `prompt` / `agent` の5種の Discriminated Union に拡張。共通フィールド `timeout` / `statusMessage` / `if`、command 型の `args` / `async` / `asyncRewake` / `shell` を追加
...
- 生成する `hooks/hooks.json` に `$schema` を出力
