...
- ワーカー（SubAgent方式）: 専門的なタスクを担当し、結果を返す
- ワーカー（Agent Team方式）: 複数のメンバーがチームとして協調し、作業とレビューを直接やり取りする

### 単一責務の原則

各 skill/agent は1つの明確な責務を持つ。

- 「計画を立てる」と「計画をレビューする」は別の担当（SubAgent方式では別agent、Agent Team方式では別teammate）
- 「実装する」と「レビューする」は別の担当
- 視点の独立性を確保し、バイアスを防ぐ

### メインスレッド vs サブエージェント vs Agent Team

- メインスレッド: ユーザーとの対話、ワークフローの制御
- サブエージェント: 隔離されたコンテキストで専門タスクを実行。結果をメインに返すのみ
- Agent Team: 複数のteammateが独立コンテキストで協調作業。teammate同士が直接通信できる
...
[CLAUDE.md]
    ...
    ↓
[Cross-cutting skill]  ──────────────────────────
    slack-notify, retrospective          横断的に適用
─────────────────────────────────────────────────
...
[Entry-point skill]
    ...
    │     │
    │     ├── [Worker skill / SubAgent方式] implement, create-pr, ...
    │     │     └── Task(subagent_type: ...) で委譲
    │     ├── [Worker skill / Agent Team方式] plan-team, ...
    │     │     └── TeamCreate + Agent で teammate を編成
    │     └── ...
    └── review-pr, lint-plugin, ...（単体完結型）
```
...
**単体完結型の例**: `review-pr`, `lint-plugin`, `improvement`
...
Worker skill には2つの実行方式がある:

SubAgent方式:
- Skill は手順・知識を定義し、Agent は `skills:` でそれをプリロードして実行する
- オーケストレーターは Agent を `Task(subagent_type: ...)` で呼び出す（Skill を直接呼ばない）
- skill-agent の紐づけが Agent 側に凝集され、オーケストレーターは「誰に依頼するか」だけを管理する

Agent Team方式:
- 1つのスキル内にチーム編成とメンバーの役割・作業内容を定義する
- オーケストレーター（メインエージェント）がリーダーとして TeamCreate + Agent でチームを編成する
- カスタムagent定義（.claude/agents/）をteammateとして使うことはできないため、agent .md ファイルは不要。メンバーの役割・作業内容はskill内に記述し、promptで指示する
- teammate同士が SendMessage で直接やり取りするため、オーケストレーターによる中継が不要

SubAgent方式を選ぶケース:
- コードベース調査やファイル編集など、自律的な判断を伴う単独作業
- コンテキスト消費が大きく、メインスレッドから隔離したい作業

Agent Team方式を選ぶケース:
- 作業者とレビュアーなど、複数の担当が直接やり取りする必要がある作業
- 独立した視点を確保しつつ、相互フィードバックで品質を高めたい作業

メインスレッド直接実行（Agent不要）のケース:
...
- メインスレッドで手順通りに実行すれば済む（例: slack-notify, retrospective）

ルール:
- Entry-point skill（オーケストレーター型）: Skillとして作成する（Agentにしない）。Subagentは更にSubagentを生成できないため
- Worker skill（SubAgent方式）: 対応するAgentを作成し、Agentの`skills:`でプリロードする。オーケストレーターはAgentを呼び出す（Skillを直接呼ばない）
- Worker skill（Agent Team方式）: agent .md ファイルは不要。メンバーの役割・作業内容をskill内に記述する。オーケストレーターがリーダーとしてskillを直接実行する
...
| `paths` | No | スキルが有効になるファイルパターン。YAML list of globs で指定 |
...
SKILL.md の本文は **作業詳細・補足説明** の2段階で構成する。概要はfrontmatterの `description` フィールドが担うため、本文には書かない。
...
{スキル実行に関する横断的な注意事項・方針。作業手順そのものではないが、実行の判断に影響する情報}
...
| 段階 | 内容 | 読み手の問い |
|------|------|-------------|
| **作業詳細** | ステップごとの具体的な作業手順 | 「具体的に何をすればよいか？」 |
| **補足説明** | 実行方針・横断的な注意事項・判断基準 | 「どういうルールで実行するのか？」 |

#### ガイドライン

- **作業詳細**と**補足説明**の間に `---`（水平線）を入れ、視覚的に段階を区切る
- 概要はfrontmatterの `description` フィールドに記載する。本文の冒頭には導入文（`$ARGUMENTS` に対して...）のみを書く
- **補足説明**は作業詳細の各ステップに横断的に適用されるルールや方針を書く。特定のステップにのみ関わる注意事項は作業詳細側に書く
- **作業詳細**の各ステップは `### Step N: {ステップ名}` で統一する
- 単体完結型スキルでもこの2段階構成を適用する。ステップが1つしかない場合でも構成は同じ
...
| `initialPrompt` | No | エージェント起動時に自動送信される最初のプロンプト。最初のターンを自動実行する |
...
|------------|------|-------|-------|--------|
| (Agent Team) | チケット調査+レビュー | - | - | linear-triage-team |
...
| (Agent Team) | 計画作成+レビュー | - | - | plan-team |
| (Agent Team) | 実装+レビュー | - | - | implement-team |
...
- 末尾に `-agent` を付ける（例: `create-pr-agent`, `split-agent`）
- 対応するスキルがある場合、スキル名と対になる名前にする（例: `create-pr` skill → `create-pr-agent`）

### Worker skill との対応（SubAgent方式）

SubAgent方式の Worker skill には対応する Agent を作成する。Agent の `skills:` で対応するスキルをプリロードし、手順・知識を自己取得する構成にする。Entry-point skill（オーケストレーター型）は Agent を `Task(subagent_type: ...)` で呼び出し、Worker skill を直接呼ばない。

```
plugins/{plugin}/skills/create-pr/SKILL.md    ← 手順・知識を定義
plugins/{plugin}/agents/create-pr-agent.md    ← skills: [create-pr] でプリロード
plugins/{plugin}/skills/dev/SKILL.md          ← Task(subagent_type: create-pr-agent) で依頼するだけ
```

### Worker skill の構成（Agent Team方式）

Agent Team方式の Worker skill は agent .md ファイルを持たない。1つのskill内にチーム編成とメンバーの役割・作業内容を定義する。カスタムagent定義（`.claude/agents/`）をteammateとして使うことは現時点ではできない（[#24316](https://github.com/anthropics/claude-code/issues/24316)）。

```
plugins/{plugin}/skills/plan-team/SKILL.md    ← チーム編成 + メンバーの役割・作業内容を定義
plugins/{plugin}/skills/dev/SKILL.md          ← plan-team skill を実行（メインエージェントがリーダー）
```

SKILL.mdの構成:
- チーム作成（TeamCreate）とメンバー起動（Agent）の手順
- 各メンバーの役割・具体的な作業手順
- リーダー（オーケストレーター）の役割
- 入出力の定義

オーケストレーターの振る舞い:
- メインエージェントがリーダーとして参加するが、コンテキスト節約のため受動的に立ち回る
- チーム作成とメンバー起動が完了したら、以降は監視に徹する。自発的な調査・作業は行わない
- 介入するのは以下の場合のみ:
  - レビューサイクルが上限（最大3回）に達した場合
  - メンバーがブロックされている場合
  - ユーザー承認が必要な場合
  - チーム作業が完了した場合（TeamDelete でクリーンアップ）
...
#### SendMessage による反復（SubAgent方式）

レビュー → 修正 → 再レビューのサイクルを `SendMessage` で実現する。初回呼び出し時に返却される `agent_id` をオーケストレーターが記録し、修正依頼時に `SendMessage(to: agent_id)` で前回のコンテキストを保持したまま継続する。

重要: SendMessage はオーケストレーター（メインスレッド）が直接呼び出すこと。Agentツールで別のagentに委譲してはならない（sub-agent は SendMessage を持たないため失敗する）。

オーケストレーターの記述例:
```markdown
出力（agent の返却値から取得）:
- PR URL
- create-pr-agent の agent_id    ← 必ず記録を明記する

#### 追加修正が必要な場合
create-pr-agent の agent_id に対して SendMessage(to: agent_id) で追加の修正・pushを依頼する。
（SendMessage はメインスレッドから直接呼ぶこと。Agentツールで委譲しない）
```

#### SendMessage による反復（Agent Team方式）

Agent Team方式ではteammate同士が SendMessage で直接やり取りするため、オーケストレーターによる中継は不要。オーケストレーターはチーム作成・メンバー起動後、監視に徹する。

コンテキスト節約のルール:
- SendMessage には判定結果（PASS/NEEDS_REVISION等）とファイルパスのみを含める
- 詳細な内容（計画本文、レビュー指摘の詳細等）はファイルに書き出し、受信側がファイルから読む
- リーダーへの通知も同様。リーダーが成果物ファイルを読むのはユーザーに提示する必要がある場合のみ

teammate間のやり取りの例:
```
planner → reviewer: 計画完成。パス: ~/claude-code-data/workflows/{id}/plan.md
reviewer → planner + リーダー: NEEDS_REVISION。レビュー結果: ~/claude-code-data/workflows/{id}/plan-review.md
planner → reviewer: 修正完了。パス: ~/claude-code-data/workflows/{id}/plan.md
reviewer → planner + リーダー: PASS。レビュー結果: ~/claude-code-data/workflows/{id}/plan-review.md
```
...
原則:
- 書き出す側の skill がファイルパスを決定し、保存後にそのパスを返す
- オーケストレーターが返却されたパスを受け取り、次の agent に渡す
- 読み込む側の skill/agent は、オーケストレーターから渡されたパスを使う（自分でパスを構築しない）
- オーケストレーターと agent ではファイル名を直接書かず、セマンティックな名前で参照する（例: `plan.md のパス` ではなく `実装計画のパス`）
...
渡す情報:
- チケットID
- 実装計画のパス（Step 2a で取得）  ← セマンティック名で中継

# skill（読み込む側）
入力として渡された実装計画のパスを読み込む
```

#### 前ステップの結果を読む場合
SKILL.mdの「入力」セクションに、オーケストレーターから渡されるファイルパスを記載する。手順内ではそのパスを使って読み込む:
```markdown
## 入力
- チケットID
- 実装計画のパス（orchestratorから渡される）

### 1. 実装計画の読み込み
入力として渡された実装計画のパスを読み込み、実装ステップを把握する。
```
...
| 保存先 | 用途 | 管理者 |
|--------|------|--------|
| `~/claude-code-data/memory/` | 作業コンテキスト（短期・中期・長期） | auto memory |
...
- `workflows/` と `state/` のデータは auto memory の管理対象外
...
**対策**: 実装担当とレビュー担当を分離し、「経緯を知らない第三者」の視点を確保（SubAgent方式では別agent、Agent Team方式では別teammate）
...
対策:
- ファイルパスはオーケストレーターが中継する。読み込む側はファイル名を知らず、入力として受け取ったパスを使う
- オーケストレーターと agent ではファイル名ではなくセマンティック名で参照する（例: `実装計画のパス`）
...
- [ ] Worker skill の実行方式を選択する（SubAgent方式 or Agent Team方式）
- [ ] Agent Team方式の場合: メンバーの役割・作業内容をskill内に定義し、オーケストレーターは受動的に監視する設計にする
- [ ] 各ステップの入出力ファイルの明確化（ファイル名は書き出す側の skill だけが知り、オーケストレーターはセマンティック名で中継する）
- [ ] レビュー系の担当は「経緯を知らない視点」で設計
- [ ] 反復サイクルの上限設定（3回など）
- [ ] 承認ポイントの適切な配置
