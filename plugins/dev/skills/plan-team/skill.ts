...
reviewer は起動後、planner に定期的に status_check を送信して進捗を確認する。
...
#### R1. 作業完了のポーリング
planner に SendMessage で \`{type: "status_check"}\` を送信し、返信を待つ。
...
NEEDS_REVISION 送信後、planner の修正完了を確認する。R1 と同じ要領で status_check を送信し、\`{status: "done"}\` を受け取ったら R2 に戻って再レビューする。レビュー結果は同じファイルパスに上書き保存する。
