...
reviewer は triager の計画完成を待つ必要があるため、起動時に「triager の計画が完成したら SendMessage で通知が届く。届くまで待機せよ」と指示する。
...
#### T6. reviewer に通知
計画の保存先パスを reviewer に SendMessage で通知する。SendMessage には完了ステータスとファイルパスのみを含める。
...
#### R1. 計画の受領待ち
triager から計画の保存先パスが SendMessage で届くまで待機する。
...
#### R6. 再レビュー
triager から修正通知を受けたら、修正された計画を再度レビューする（R2 に戻る）。レビュー結果は同じファイルパスに上書き保存する。
