...
describe("generateTeamContent", () => {
  ...
  it("メンバー起動セクションに全メンバーの起動パラメータが含まれる", () => {
    ...
    expect(result).toContain("implementer と reviewer を同時に起動する（並列）");
  ...
  it("ポーリング関係の説明が生成される", () => {
    ...
    expect(result).toContain("reviewer は起動後、implementer に定期的に status_check を送信して進捗を確認する。");
  });
  ...
  it("requiresUserApproval が true の場合、ユーザー承認フローが含まれる", () => {
    const result = generateTeamContent(makeInput({ requiresUserApproval: true }));

    expect(result).toContain("レビューPASS後、成果物をユーザーに提示して承認を得る");
    expect(result).toContain("implementer に SendMessage で修正を依頼する");
  });

  it("requiresUserApproval が false/undefined の場合、ユーザー承認フローが含まれない", () => {
    const result = generateTeamContent(makeInput({ requiresUserApproval: false }));
  ...
  it("statusCheckResponder のteammateにstatus_check応答ルールが生成される", () => {
    ...
    expect(result).toContain('reviewer から `{type: "status_check"}` を受信する場合がある');
